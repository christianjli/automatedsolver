...
indices.
    ...
    for i, row in enumerate(puzzle):
        ...
        for j, char in enumerate(row):
            ...
            if char.isalnum(): # flow endpoint
                if char in colors:
                    color = colors[char] # number of time the color shows up, tracked in dictionary
                    if color_count[color]: # seen 3rd+ time, invalid
                        print ('{}:{}:{} too many {} already'.format(
                            filename, i+1, j, char))
                        return None, None
                    color_count[color] = 1 # increment color count at index
                else:
                    color = len(colors)
                    colors[char] = color # add entry in dictionary, value of color based on length of dictionary
                    color_count.append(0) # 0 is the first time its seen, 1 is the second

    # check parity
    for char, color in colors.items():
        # print(char, color)
        if not color_count[color]: # check if any 0 in color_count, means we only see a color once
    ...
    puzzle, colors = repair_colors(puzzle, colors) # changes A, B, C to standard colors
    return puzzle, colors
...
encodes a single color in a one-hot fashion.
    ...
    # print("lol", num_colors, puzzle, type(puzzle))
    ...
    for i, j, char in explode(puzzle):
        ...
        if char.isalnum(): # flow endpoint
            ...
            endpoint_color = colors[char] # index value
            # print(char, endpoint_color)

            # color in this cell is this one
            clauses.append([color_var(i, j, endpoint_color)]) # add unique identifier with color, cell coords
            # print(clauses)

            # color in this cell is not the other ones
            for other_color in range(num_colors):
                if other_color != endpoint_color:
                    clauses.append([-color_var(i, j, other_color)]) # for all other colors, add negative clause so they can't be on this coord

            # gather neighbors' variables for this color
            neighbor_vars = [color_var(ni, nj, endpoint_color) for
                             _, ni, nj in valid_neighbors(size, i, j)] # path must continue to a valid neighbor

            # one neighbor has this color
            clauses.append(neighbor_vars)

            # no two neighbors have this color
            clauses.extend(no_two(neighbor_vars)) # make sure it only goes to one neighbor with no_two, i.e. can't go up and down

        else:

            # one of the colors in this cell is set
            clauses.append([color_var(i, j, color)
                            for color in range(num_colors)]) # empty cell, all colors can fill this cell

            # no two of the colors in this cell are set
            cell_color_vars = (color_var(i, j, color) for
                               color in range(num_colors))

            clauses.extend(no_two(cell_color_vars)) # ensure only one color fills this cell with no_two

    return clauses

######################################################################

def make_dir_vars(options, puzzle, start_var, colors):
    ...
    for i, j, char in explode(puzzle):
        ...
        neighbor_bits = (dir_bit for (dir_bit, ni, nj)
                         in valid_neighbors(size, i, j)) # get valid directional bits of each empty cell
        # print(i, j, char)
        # print(list(dir_bit for (dir_bit, ni, nj) in valid_neighbors(size, i, j)))

        # OR them all together
        # print(operator.or_, neighbor_bits, 0)
        cell_flags = reduce(operator.or_, neighbor_bits, 0) # results in value to represent all possible directions
        # print(cell_flags, "cell flags")
        # create a lookup for dir type vars in this cell
        dir_vars[i, j] = dict() # new dict within dict dir_vars given coords

        for code in DIR_TYPES:
            # only add var if cell has correct flags (i.e. if cell has
            # TOP, BOTTOM, RIGHT, don't add LR).
            # print(cell_flags, type(cell_flags))
            # print(code, type(code))
            if cell_flags & code == code: # if current dir type is represented in cell_flags value (Left/Right, Top/Bottom, etc) max of 6
                # print("fewfkeowpfkewop")
                num_dir_vars += 1
                # print(start_var, num_dir_vars, i, j, char, cell_flags, code)
                dir_vars[i, j][code] = start_var + num_dir_vars # start variable is last unique id from color_var, so we continue from there
        # print("done")
    # print(dir_vars, "dir vars")
    # print(num_dir_vars, start_var + num_dir_vars)
...
directions imply color matching with neighbors.
    ...
    for i, j, char in explode(puzzle):
        ...
        cell_dir_dict = dir_vars[(i, j)] # dictionary of dir: id from dir_vars in previous function
        cell_dir_vars = cell_dir_dict.values() # all id values from cell_dir_dict
        # print(cell_dir_dict, cell_dir_vars)
            ...
            for dir_bit, n_i, n_j in all_neighbors(i, j):
                ...
                for dir_type, dir_var in cell_dir_dict.items():
                    ...
                    if dir_type & dir_bit:
                        ...
                        dir_clauses.append([-dir_var, -color_1, color_2]) # because we already ensured no_two, if directions match, colors match
...
possibly negated.
 ...
 column j.
    ...
    color_clauses = make_color_clauses(puzzle,
                                       colors,
                                       color_var) # all clauses for SAT
    # print("here?")
    dir_vars, num_dir_vars = make_dir_vars(options, puzzle, num_color_vars, colors) # dir_vars is dictionary of coord:dir:id
    # print("not here")
    dir_clauses = make_dir_clauses(puzzle, colors,
                                   color_var, dir_vars) # all directional clauses for SAT
...
2D array of (color, direction-type) pairs.
    ...
    for i, row in enumerate(puzzle):
        ...
        for j, char in enumerate(row):
            ...
            for color in range(num_colors):
                # print(cell_color)
                if color_var(i, j, color) in sol: # only true for one color, which we assign to cell_color
                    # print("booooom", cell_color == -1)
                    assert cell_color == -1
                    cell_color = color # match 
                    # print(cell_color, color)

            assert cell_color != -1 # ensure cell_color isn't -1
            ...
            if not char.isalnum(): # not a flow endpoint
                ...
                for dir_type, dir_var in dir_vars[i, j].items():
                    if dir_var in sol: # only one directional per cell in solution, assign it to cell_dir_type
                        assert cell_dir_type == -1
                        cell_dir_type = dir_type

                assert cell_dir_type != -1

            decoded_row.append((cell_color, cell_dir_type)) # cell_dir_type stays -1 if endpoint
...
boolean flag indicating if a cycle was detected.
    ...
    while True:
        ...
        for dir_bit, n_i, n_j in valid_neighbors(size, cur_i, cur_j):
            ...
            if ((dir_type >= 0 and (dir_type & dir_bit)) or
                    (dir_type == -1 and n_dir_type >= 0 and
                     ...
                     # if neighbor and decoded same direction, cell points at neighbor
                     # or dir_type == -1 (endpoint) and next dir matches opposite of neighbors dir_bit, neighbor points at cell
        ...
        if not advanced: # went through all neighbors and didn't advance or found cycle
            break
    # print(run, is_cycle)
    return run, is_cycle
...
to prevent them.
    ...
    for i, j in itertools.product(range(size), range(size)):
        ...
        if not visited[i][j]:
            ...
            # get the path
            run, is_cycle = make_path(decoded, visited, i, j) # run is list of (x, y) coords that make path
            assert is_cycle
            ...
            for r_i, r_j in run:
                _, dir_type = decoded[r_i][r_j] # color, dir_type
                dir_var = dir_vars[r_i, r_j][dir_type] # get unique SAT variable
                clause.append(-dir_var) # prevent it from taking the same direction
...
needed.
    ...
    while True:
        ...
        if not isinstance(sol, list): # check if it returns a list, otherwise no valid solution
            decoded = None
            all_decoded.append(decoded)
            break

        decoded = decode_solution(puzzle, colors, color_var, dir_vars, sol) # turn SAT solution with IDs back to understandable pathways
        all_decoded.append(decoded)

        extra_clauses = detect_cycles(decoded, dir_vars) # if cycles detected, add extra clauses
    ...
    if not options.quiet:
        ...
        if decoded is None:
            print ('solver returned {} after {:,} cycle '\
                'repair(s) and {:.3f} seconds'.format(
                    str(sol), repairs, solve_time))

        else:
            print ('obtained solution after {:,} cycle repair(s) '\
...
def pyflow_solver_main():
    ...
    for filename in options.filenames:
        ...
        color_var, dir_vars, num_vars, clauses, reduce_time = \
            reduce_to_sat(options, puzzle, colors) # reduce to SAT outputs
        ...
        if not result_char in stats: # if result not seen before, add to stats dictionary
            stats[result_char] = cur_stats
            # print(stats)
        else:
            for key in cur_stats.keys():
                stats[result_char][key] += cur_stats[key] # add new stats to dictionary under appropriate result
